...
export class ArrayAssertion<T> extends IntuitiveAssertions<Array<T>> {
    ...
    public beEmpty(): Continuance<this> {
        ...
        .throwWithMessage(
            !this.opposite
                ? `Expected collection to be empty, but found [ ${processArr(this.array)} ] with length '${this.array.length}'`
    ...
    public haveLength(expectedLength: number): Continuance<this> {
        Execute.stuff.checkCondition(
            !this.opposite
                ? this.array.length === expectedLength
                : this.array.length !== expectedLength
        )
        .throwWithMessage(
            !this.opposite
    ...
    public containsType(type: 'number' | 'string' | 'boolean' | 'object' | 'undefined'): Continuance<this> {
        ...
        if (
            ...
            Execute.stuff.checkCondition(false)
                .throwWithMessage(
                !this.opposite
    ...
    public startsWith(arr: Array<boolean | object | null | undefined | string | number>): Continuance<this> {
        ...
        Execute.stuff.checkCondition(startsWith)
            .throwWithMessage(
                !this.opposite
    ...
    public endsWith(arr: Array<T>): Continuance<this> {
        ...
        Execute.stuff.checkCondition(endsWith)
            .throwWithMessage(
                !this.opposite
    ...
    public beSortedInASC(): Continuance<this> {
        ...
        if (this.array.find(el => { return typeof el === 'number' })) {
            sortedArray = [...this.array].sort(function(a: any, b: any) {return a - b });
        } else {
        ...
        Execute.stuff.checkCondition(isSortedInASC)
            .throwWithMessage(
                !this.opposite
    ...
    public beSortedInDESC(): Continuance<this> {
        ...
        if (this.array.find(el => { return typeof el === 'number' })) {
            sortedArray = [...this.array].sort(function(a: any, b: any) {return a - b }).reverse();
        } else {
    ...
    public equalsTo(arr: Array<T>): Continuance<this> {
        const isEqual = Array.isArray(this.array) &&
            Array.isArray(arr) &&
