...
export class ObjectAssertion {
    ...
    public haveLength(length: number): Continuance<this> {
        Execute.stuff.checkCondition(
                !this.opposite
                    ? Object.keys(this.subject).length === length
                    : Object.keys(this.subject).length !== length
            )
            .throwWithMessage(
                !this.opposite
    ...
    public haveLengthEqualOrGreaterThan(length: number): Continuance<this> {
        Execute.stuff.checkCondition(
            ...
            .throwWithMessage(
                !this.opposite
                    ? `Expected collection to have length equal or greater than '${length}', but found ${Object.keys(this.subject).length}`
                    : `Expected collection to not have length equal or greater than '${length}', but found ${Object.keys(this.subject).length}`);
    ...
    public haveLengthEqualOrLesserThan(length: number): Continuance<this> {
        Execute.stuff.checkCondition(
            ...
            .throwWithMessage(
                !this.opposite
                    ? `Expected collection to have length equal or lesser than '${length}', but found ${Object.keys(this.subject).length}`
                    : `Expected collection to not have length equal or lesser than '${length}', but found ${Object.keys(this.subject).length}`
            );
        ...
        if (
            ...
            Execute.stuff.throwWithMessage(
                !this.opposite
                    ...
                    : `Expected collection does not contain null or undefined, but found ${JSON.stringify(this.subject?.valueOf())}`
            );
        }
        ...
        if (isContainsProp) {
            ...
            Execute.stuff.throwWithMessage(
                !this.opposite
                    ...
                    : `Expected collection does not contain '${expectedKey}' key, but found ${JSON.stringify(this.subject?.valueOf())}`
            );
        }
